...
DEFINE_MLOGINSTANCE_CLASS(YUVReaderElement, "YUVReaderElement");

YUVReaderElement::YUVReaderElement(ElementType type,
...
YUVReaderElement::~YUVReaderElement()
{
    if (user_dec_) {
        user_dec_->Close();
...
bool YUVReaderElement::Init(void *cfg, ElementMode element_mode)
{
    element_mode_ = element_mode;
    ElementCfg *ele_param = static_cast<ElementCfg *>(cfg);

    if (NULL == ele_param) {
        MLOG_ERROR("YUV reader initialize params is null\n");
        return false;
    }

    measurement = ele_param->measurement;
    user_dec_ = new YUVReaderPlugin();
...
int YUVReaderElement::Recycle(MediaBuf &buf)
{
    recycle(buf);
...
mfxStatus YUVReaderElement::InitDecoder()
{
    mfxStatus sts = MFX_ERR_NONE;

    sts = user_dec_->DecodeHeader(input_raw_, &mfx_video_param_);
    if (MFX_ERR_NONE != sts) {
        MLOG_ERROR("YUV decodeHeader return %d, failed.\n", sts);
        return sts;
    }
...
int YUVReaderElement::HandleProcessDecode()
{
    mfxStatus sts = MFX_ERR_NONE;
    MediaBuf buf;
        ...
        if (!codec_init_) {
            ...
            if (MFX_ERR_MORE_DATA == sts) {
                ...
                if (measurement) {
                    ...
                    einfo.mChannelNum = YUVReaderElement::mNumOfDecChannels;
                    YUVReaderElement::mNumOfDecChannels++;
                    //if (input_mp_) {
...
int YUVReaderElement::HandleProcess()
{
    return HandleProcessDecode();
}
unsigned int YUVReaderElement::mNumOfDecChannels = 0;
