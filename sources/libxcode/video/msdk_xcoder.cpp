...
int MsdkXcoder::InitVaDisp()
{
   ...
   WaitForSingleObject(va_mutex_, INFINITE);

#if MFX_D3D11_SUPPORT
    if (D3D11_MEMORY == m_memType)
        hwdev_ = new CD3D11Device();
    else
#endif // #if MFX_D3D11_SUPPORT
        hwdev_ = new CD3D9Device();

    if (NULL == hwdev_) {
        ReleaseMutex(va_mutex_);
        return MFX_ERR_MEMORY_ALLOC;
    }

    sts = hwdev_->Init(NULL, 0, MSDKAdapter::GetNumber());
    if(MFX_ERR_NONE != sts) {
        ReleaseMutex(va_mutex_);
        return sts;
    }
    ...
    sts = hwdev_->GetHandle(hdl_t, &(mfxHDL)MsdkXcoder::va_dpy_);
    if(MFX_ERR_NONE != sts) {
        ReleaseMutex(va_mutex_);
        return sts;
    }
    ...
    pd3dAllocParams->pManager = reinterpret_cast<IDirect3DDeviceManager9 *>(MsdkXcoder::va_dpy_);
    m_pD3DAllocatorParams = pd3dAllocParams;
...
int MsdkXcoder::InitVaDisp()
{
    ...
    if (m_bRender_ == false) {
        ...
        if (VA_STATUS_SUCCESS != ret) {
            //if failed, try to use renderD128
            if (NULL != va_dpy_) {
                vaTerminate(va_dpy_);
                va_dpy_ = NULL;
            }
            ...
            if (-1 == dri_fd_) {
                MLOG_FATAL("Open dri/renderD128 failed!\n");
                pthread_mutex_unlock(&va_mutex_);
                return -1;
            }
            va_dpy_ = vaGetDisplayDRM(dri_fd_);
...
#endif // #if MFX_D3D11_SUPPORT
            MFX_HANDLE_D3D9_DEVICE_MANAGER;
        sts = pSession->SetHandle(hdl_t, MsdkXcoder::va_dpy_);
#elif defined (PLATFORM_OS_LINUX)
...
void MsdkXcoder::CreateSessionAllocator(MFXVideoSession **session, GeneralAllocator **allocator)
{
    ...
    *allocator = NULL;
    ...
    if (libType_ != MFX_IMPL_SOFTWARE) {
#if defined (PLATFORM_OS_WINDOWS)
        sts = pAllocator->Init(m_pD3DAllocatorParams);
        if (sts != MFX_ERR_NONE) {
        ...
        sts = pAllocator->Init(m_pAllocatorParams);
        if (sts != MFX_ERR_NONE) {
            MLOG_ERROR("Init allocator failed\n");
            delete pAllocator;
            return;
...
#if defined (PLATFORM_OS_WINDOWS)
//void MsdkXcoder::CreateEncoderSessionAllocator(MFXVideoSession **session, D3DFrameAllocator **allocator)
void MsdkXcoder::CreateEncoderSessionAllocator(MFXVideoSession **session, MFXFrameAllocator **allocator)
{
    mfxStatus sts = MFX_ERR_NONE;
    BaseFrameAllocator * pAllocator = NULL;
    *session = NULL;
    *allocator = NULL;

    if (libType_ == MFX_IMPL_SOFTWARE) {
        pAllocator = new SysMemFrameAllocator();
        if (!pAllocator) {
            MLOG_ERROR("Create software allocator failed\n");
            return;
        }
    } else {
        pAllocator = new D3DFrameAllocator;
        if (!pAllocator) {
            MLOG_ERROR("Create D3D allocator failed\n");
            return;
        }
...
void MsdkXcoder::CreateEncoderSessionAllocator(MFXVideoSession **session, MFXFrameAllocator **allocator)
{
    ...
    *allocator = NULL;
    ...
    if (libType_ == MFX_IMPL_SOFTWARE) {
        SysMemFrameAllocator *pAlloc = new SysMemFrameAllocator;
        if (!pAlloc) {
            MLOG_ERROR("Create software allocator failed\n");
            return;
        }
        sts = pAlloc->Init(m_pAllocatorParams);
        if (sts != MFX_ERR_NONE) {
            MLOG_ERROR("Init allocator failed\n");
            delete pAlloc;
            return;
        ...
        pAllocator = new vaapiFrameAllocator;
        if (!pAllocator) {
            MLOG_ERROR("Create allocator failed\n");
            return;
        }
...
void MsdkXcoder::ReadDecConfig(DecOptions *dec_cfg, void *cfg)
{
    ...
    decCfg->isMempool = dec_cfg->isMempool;
    if (dec_cfg->input_codec_type != CODEC_TYPE_VIDEO_YUV
        && dec_cfg->input_codec_type != CODEC_TYPE_VIDEO_STRING
        && dec_cfg->input_codec_type != CODEC_TYPE_VIDEO_PICTURE) {
        if (decCfg->isMempool)
            decCfg->input_mp = dec_cfg->inputMempool;
        else
            decCfg->input_stream = dec_cfg->inputStream;
    }
    decCfg->measurement = dec_cfg->measurement;
    //decCfg->reader = dec_cfg->reader;
    decCfg->extDecParams.have_NoVpp = dec_cfg->have_NoVpp;
    decCfg->extDecParams.suggestSurfaceNum = dec_cfg->suggestSurfaceNum;
    decCfg->libType = libType_;
}
...
void MsdkXcoder::ReadVppConfig(VppOptions *vpp_cfg, void *cfg)
{
    ...
    vppCfg->measurement = vpp_cfg->measurement;

    // vppCfg->extVppParams.bLABRC = vpp_cfg->bLABRC;
    // vppCfg->extVppParams.nLADepth = vpp_cfg->nLADepth;
    vppCfg->extVppParams.nSuggestSurface = vpp_cfg->nSuggestSurface;

    vppCfg->vpp_denoise.enabled = vpp_cfg->denoiseEnabled;
    if (vppCfg->vpp_denoise.enabled) {
        if (vpp_cfg->denoiseFactor <= 100) {
            // if in range [0,100]
            vppCfg->vpp_denoise.factor = vpp_cfg->denoiseFactor;
        } else {
            // else using default value
            vppCfg->vpp_denoise.factor = 0xFFFF;
        }
    }

    vppCfg->libType = libType_;
}
...
void MsdkXcoder::ReadEncConfig(EncOptions *enc_cfg, void *cfg)
{
    ...
    if (MFX_CODEC_MPEG2 == encCfg->EncParams.mfx.CodecId ||
                MFX_CODEC_HEVC == encCfg->EncParams.mfx.CodecId ||
                MFX_CODEC_AVC == encCfg->EncParams.mfx.CodecId) {
        encCfg->EncParams.mfx.TargetUsage = enc_cfg->targetUsage==0? 4: enc_cfg->targetUsage;
        encCfg->EncParams.mfx.GopRefDist = enc_cfg->gopRefDist==0?4:enc_cfg->gopRefDist;
        encCfg->EncParams.mfx.GopPicSize = enc_cfg->intraPeriod==0? 30:enc_cfg->intraPeriod;
        encCfg->EncParams.mfx.GopOptFlag = enc_cfg->gopOptFlag;
        encCfg->EncParams.mfx.IdrInterval= enc_cfg->idrInterval;
        encCfg->EncParams.mfx.NumSlice = enc_cfg->numSlices==0?1:enc_cfg->numSlices;
        ...
        } else if (enc_cfg->ratectrl == MFX_RATECONTROL_ICQ){
            encCfg->EncParams.mfx.ICQQuality = enc_cfg->qpValue;
        }else{
            encCfg->EncParams.mfx.TargetKbps = enc_cfg->bitrate;
        }
    ...
    encCfg->extParams.nMbPerSlice = enc_cfg->nMbPerSlice;
    encCfg->extParams.bRefType = enc_cfg->bRefType;
    encCfg->extParams.pRefType = enc_cfg->pRefType;
    encCfg->extParams.GPB = enc_cfg->GPB;
#if MSDK_ROI_SUPPORT && MFX_VERSION >= 1022
    encCfg->extRoiData = enc_cfg->extRoiData;
#endif // MSDK_ROI_SUPPORT && MFX_VERSION >= 1022
#if (MFX_VERSION >= 1025)
    encCfg->extParams.nMFEFrames = enc_cfg->nMFEFrames;
    encCfg->extParams.nMFEMode = enc_cfg->nMFEMode;
    encCfg->extParams.nMFETimeout = enc_cfg->nMFETimeout;
#endif
    encCfg->extParams.nRepartitionCheckEnable = enc_cfg->nRepartitionCheckEnable;
#if defined(MSDK_HEVC_FEI) || defined(MSDK_AVC_FEI)
    encCfg->extParams.bPreEnc = enc_cfg->bPreEnc;
    encCfg->extParams.bEncode = enc_cfg->bEncode;
    encCfg->extParams.bENC = enc_cfg->bENC;
    encCfg->extParams.bPAK = enc_cfg->bPAK;
    encCfg->extParams.bQualityRepack = enc_cfg->bQualityRepack;
    encCfg->extParams.mvinStream = enc_cfg->mvinStream;
    encCfg->extParams.mvoutStream = enc_cfg->mvoutStream;
    encCfg->extParams.mbcodeoutStream = enc_cfg->mbcodeoutStream;
    encCfg->extParams.weightStream = enc_cfg->weightStream;

    encCfg->extParams.fei_ctrl = enc_cfg->fei_ctrl;
#endif

    encCfg->measurement = enc_cfg->measurement;
    encCfg->libType = libType_;
}

void MsdkXcoder::SetLibraryType(int type)
{
    libType_ = type;
}
...
int MsdkXcoder::Init(DecOptions *dec_cfg, VppOptions *vpp_cfg, EncOptions *enc_cfg, CodecEventCallback *callback)
{
    ...
    if (!dec_cfg || (!enc_cfg && (vpp_cfg && vpp_cfg->colorFormat != MFX_FOURCC_RGB4))) {
    ...
    if (m_bRender_) {
        ...
        if (enc_cfg) {
            ...
            MLOG_ERROR("Enc configure is NULL in Init\n");
            return -1;
        }
    }
...
#if 0
#if defined (PLATFORM_OS_WINDOWS)
    D3DFrameAllocator *pAllocatorEnc = NULL;
#elif defined (PLATFORM_OS_LINUX)
    vaapiFrameAllocator *pAllocatorEnc = NULL;
#endif
#endif
    MFXFrameAllocator *pAllocatorEnc = NULL;
    //render session and allocator
    MFXVideoSession *render_session = NULL;
    GeneralAllocator *pAllocatorRender = NULL;
    if (!m_bRender_) {
        CreateEncoderSessionAllocator(&enc_session, &pAllocatorEnc);
    ...
    if (MFX_CODEC_VP8 == in_file_type) {
        ElementType type = GetElementType(DECODER, in_file_type, !dec_cfg->vp8UseHWDec);
        dec_ = ElementFactory::createElement(type, dec_session, pAllocatorDec, callback_);
    } else if (MFX_CODEC_STRING == in_file_type) {
        dec_ = ElementFactory::createElement(ELEMENT_STRING_DEC, dec_session, pAllocatorDec);
    } else if (MFX_CODEC_JPEG == in_file_type) {
        dec_ = ElementFactory::createElement(ELEMENT_SW_DEC, dec_session, pAllocatorDec);
    } else if (MFX_CODEC_PICTURE == in_file_type) {
        dec_ = ElementFactory::createElement(ELEMENT_BMP_DEC, dec_session, pAllocatorDec);
    } else {
        dec_ = ElementFactory::createElement(ELEMENT_DECODER, dec_session, pAllocatorDec, callback_);
    }
    if (dec_) {
    ...
    if (vpp_cfg) {
        vpp_ = ElementFactory::createElement(ELEMENT_VPP, vpp_session, pAllocatorVpp);
        if (vpp_) {
    ...
    if (!m_bRender_) {
        ElementType type;
#if defined(MSDK_HEVC_FEI) || defined(MSDK_AVC_FEI)
        if (!enc_cfg->bPreEnc && !enc_cfg->bEncode && !enc_cfg->bENC && !enc_cfg->bPAK) {
#endif
            type = GetElementType(ENCODER, out_file_type, enc_cfg->swCodecPref);
#if defined(MSDK_HEVC_FEI) || defined(MSDK_AVC_FEI)
        } else {
            type = GetElementType(FEI_ENCODER, out_file_type, enc_cfg->swCodecPref);
            if (enc_cfg->bPreEnc && enc_cfg->bENC) {
                MFXVideoSession *preenc_session = NULL;
                preenc_session = GenMsdkSession();
                if(!preenc_session){
                    MLOG_ERROR("Init Session failed\n");
                    return -1;
                }
                preenc_session->SetFrameAllocator(pAllocatorEnc);
                encCfg.extParams.pPreencSession = preenc_session;
                main_session_->JoinSession(*preenc_session);
            }
        }
#endif
        enc_ = ElementFactory::createElement(type, enc_session, pAllocatorEnc);
#ifdef ENABLE_HEVC
        if (ELEMENT_HEVC_ENC == type) {
            if (0 != enc_cfg->numTileRows) {
                dynamic_cast<HevcEncoderElement *>(enc_)->SetTiles(enc_cfg->numTileRows, enc_cfg->numTileColumns);
            }
        }
#endif
        if (enc_) {
        ...
        render_ = ElementFactory::createElement(ELEMENT_RENDER, render_session, pAllocatorRender);
        if (!render_) {
            MLOG_ERROR("Creater render failed.");
            return -1;
        }
...
int MsdkXcoder::AppendEncoderROI(void *output_handle, std::vector<EncoderROI> *roi_list)
{
    Locker<Mutex> lock(mutex);
...
int MsdkXcoder::SetComboType(ComboType type, void *vpp_handle, void* master)
{
    ...
    for (vpp_it = vpp_list_.begin(); vpp_it != vpp_list_.end(); ++vpp_it) {
        if (*vpp_it == vpp_handle) {
            ...
            assert(ret_val == 0);
            if (0 != ret_val) {
                MLOG_ERROR(" Vpp configure error\n");
            }
            break;
        }
    }
...
int MsdkXcoder::AttachInput(DecOptions *dec_cfg, void *vppHandle)
{
    ...
    if (MFX_CODEC_VP8 == decCfg.DecParams.mfx.CodecId) {
        ElementType type = GetElementType(DECODER, decCfg.DecParams.mfx.CodecId, !dec_cfg->vp8UseHWDec);
        dec_ = ElementFactory::createElement(type, dec_session, pAllocatorDec, callback_);
        ...
        dec_ = ElementFactory::createElement(ELEMENT_YUV_READER, dec_session, pAllocatorDec);
#endif
    } else {
        dec_ = ElementFactory::createElement(ELEMENT_DECODER, dec_session, pAllocatorDec, callback_);
    }
...
void MsdkXcoder::StringOperateAttach(DecOptions *dec_cfg, void *vppHandle, unsigned long time)
{
    ...
    if (frame_num <= vppframe_num) {
        ...
        while (frame_num > (vppframe_num + 1)) {
            if (false == vpp->IsRunning()) {
                MLOG_WARNING("The specified attached time exceeds video stream duration.\n");
                return;
            }
            usleep(1000);
...
void MsdkXcoder::StringOperateDetach(DecOptions *dec_cfg, void *vppHandle, unsigned long time)
{
    ...
    if (frame_num <= vppframe_num) {
        ...
        while (frame_num > (vppframe_num + 1)) {
            if (false == vpp->IsRunning()) {
                MLOG_WARNING("The specified detached time exceeds video stream duration.\n");
                return;
            }
            usleep(1000);
            vppframe_num = vpp->GetNumOfVppFrm();
...
void MsdkXcoder::StringOperateChange(DecOptions *dec_cfg, void *vppHandle, unsigned long time)
{
    ...
    if (frame_num <= vppframe_num) {
        ...
        while (frame_num > (vppframe_num + 1)) {
            if (false == vpp->IsRunning()) {
                MLOG_WARNING("The specified changed time exceeds video stream duration.\n");
                return;
            }
            usleep(1000);
            vppframe_num = vpp->GetNumOfVppFrm();
...
void MsdkXcoder::PicOperateAttach(DecOptions *dec_cfg, void *vppHandle, unsigned long time)
{
    ...
    if (0 == time) {
        MLOG_WARNING("Vpp does not need to process frames Attach at once.\n");
        AttachInput(dec_cfg, vppHandle);
        return;
    }
...
void MsdkXcoder::CleanUpResource()
{
    ...
    std::vector<BaseFrameAllocator*>::iterator pAllocArray_i;
...
#if defined(MSDK_HEVC_FEI) || defined(MSDK_AVC_FEI)
    }else if (type == FEI_ENCODER) {
        switch (codec_type) {
        case MFX_CODEC_AVC:
            ret = ELEMENT_AVC_FEI_ENCODER;
            break;
        case MFX_CODEC_HEVC:
            ret = ELEMENT_HEVC_FEI_ENCODER;
            break;
        default:
            ret = ELEMENT_ENCODER;
            break;
...
void MsdkXcoder::AttachVpp(VppOptions *vpp_cfg, EncOptions *enc_cfg)
{
    ...
    if (vpp_ && vpp_session) {
        ...
        enc_multimap_.insert(std::make_pair(nullptr, enc_));
    }
    main_session_->JoinSession(*enc_session);
    ...
    for (dec_dis_i = dec_dis_map_.begin(); dec_dis_i != dec_dis_map_.end(); ++dec_dis_i) {
        if (vpp_)
            ...
            (dec_dis_i->second)->LinkNextElement(enc_);
    }
    if (vpp_) {
        vpp_->LinkNextElement(vpp_dis_);
        vpp_dis_->LinkNextElement(enc_);
...
/*
 * Insert RL frame for encoder "encHandle". If encHandle is null, apply this to all ELEMENT_ENCODER
 * Returns:  0 - successful
 *          -1 - XCoder is not initialized yet
 *          -2 - failed, invalid call
 *          -3 - failed with other cause.
 *          -4 - failed, invalid encoder handle
 */
int MsdkXcoder::MarkRL(void *encHandle, unsigned int refNum)
{
    Locker<Mutex> lock(mutex);
    if (!done_init_) {
        MLOG_ERROR(" MsdkXcoder is not initialized yet\n");
