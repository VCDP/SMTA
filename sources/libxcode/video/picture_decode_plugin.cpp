...
mfxStatus PicDecPlugin::UploadToSurface(unsigned char *yuv_buf, mfxFrameSurface1 *surface_out)
{
    ...
    /* Y plane */
    for (row = 0; row < surface_out->Info.CropH; row++) {
        fast_copy(y_dst, surface_out->Info.CropW, y_src, surface_out->Info.CropW);
    ...
    if (surface_out->Info.FourCC == MFX_FOURCC_NV12) {
        for (row = 0; row < (surface_out->Info.CropH / 2); row++) {
            fast_copy(uv_dst, surface_out->Info.CropW, uv_src, surface_out->Info.CropW);
