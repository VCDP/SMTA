...
mfxStatus YUVWriterPlugin::GetInputYUV(mfxFrameSurface1 *surf)
{
    ...
    if (surf && frame_buf_) {
        ...
        if (MFX_FOURCC_NV12 == surf->Info.FourCC) {
            ...
            fast_copy(swap_buf_, m_height_ * surf->Data.Pitch, sY, m_height_ * surf->Data.Pitch);
            fast_copy(swap_buf_ + m_height_ * surf->Data.Pitch, m_height_ / 2 * surf->Data.Pitch, sUV, m_height_ / 2 * surf->Data.Pitch);
            ...
            for (unsigned int j = 0; j < m_height_ >> 1; j++) {
                fast_copy(dY, m_width_, sY, m_width_);
                dY += m_width_;
                sY += surf->Data.Pitch;
                fast_copy(dY, m_width_, sY, m_width_);
                dY += m_width_;
    ...
    fast_copy(m_BS->Data + m_BS->DataLength, buff_size, frame_buf_, buff_size);
